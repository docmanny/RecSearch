...
"""
def RecBlast(seqfile, species2, database1, filetype="fasta",species1="Homo Sapiens",blasttype='blastp',localblast1=True,
             localblast2=False,database2="refseq_proteins",evalue=0.001,identitiesperc=0.75,scoreperc=0.75,
             lengthperc=0.75,idthres='Score'):
    import os
    ...
    for index, seq_record in enumerate(SeqIO.parse(seqfile, filetype)):
        ...
        # outfile is the file where the final Blast output will go to for this particular sequence. This is the annotated
        ...
        outfile = os.path.join(os.getcwd(), "{}_recblast_out".format(species2), "{0}_{1}.fasta".format(blasttype,seq_record.name))
        ...
        with open(outfile, "w+") as blastout:
            ...
            if localblast1:
                ...
                with open(os.path.join(os.getcwd(), "recblast_out", "{1}to{0}blast_{2}.xml".format(species2, species1,
            ...
            blasthits = open("{1}to{0}blast_{2}.xml".format(species2, species1, seq_record.name),"r")
                ...
                for alignment in blastrecord.alignments:
                    ...
                    for hsp in alignment.hsps:
                        if (hsp.score >= (scoreperc * topscore) & hsp.expect <= evalue &
                            ...
                            recblasthits = open("{0}to{1}blast_{2}.xml".format(species2, species1, seq_record.name))
